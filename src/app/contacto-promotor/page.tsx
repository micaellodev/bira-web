...
export default function ContactoPromotorPage() {
    const [promotores, setPromotores] = useState<PromotorContacto[]>([]);
    ...
    return (
        <main className="relative flex flex-col items-center gap-6 text-center min-h-screen w-full justify-center bg-black text-white px-4 py-8 overflow-hidden">
            ...
            {/* Content */}
            <div className="relative z-10 flex flex-col items-center gap-8 w-full max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                {/* Title */}
                <h1 className="text-3xl sm:text-4xl font-bold text-white animate-in fade-in slide-in-from-bottom-2 duration-700 delay-100">
                    Encuentra y Contacta con tu Promotor
                </h1>
                <p className="text-gray-400 text-base sm:text-lg max-w-2xl animate-in fade-in slide-in-from-bottom-2 duration-700 delay-150">
                    Selecciona tu promotor y obtén tu código de invitación el evento
                </p>

                {error && (
                    <div className="w-full max-w-md bg-red-500/10 border border-red-500/30 rounded-lg p-4 backdrop-blur-sm">
                ...
                {promotores.length > 0 ? (
                        ...
                        {promotores.map((promotor) => (
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <p className="text-gray-400 text-xs uppercase tracking-wide">Promotor</p>
                                ...
                                <Button
                                    ...
                                    onClick={() => window.open(`https://wa.me/+51${formatWhatsAppNumber(promotor.telefono)}`, '_blank')}
                                >
                                    <svg
                    ...
                    !error && (
                        <div className="w-full max-w-md bg-white/5 border border-white/20 backdrop-blur-sm rounded-xl p-8">
                ...
                <div className="w-full max-w-md mt-8 pt-6 border-t border-white/10 animate-in fade-in duration-700 delay-400">
                    <p className="text-gray-400 text-sm mb-4 text-center">Comparte con tus amigos</p>
                    <div className="flex justify-center gap-4">
                        {/* WhatsApp Share */}
                        <button
                            onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent('¡Bira Party te invita a asistir a su evento! ' + 'https://biraparty.lat')}`, '_blank')}
                        ...
                        {/* Instagram Share */}
                        <button
                            onClick={() => window.open('https://www.instagram.com/biraparty', '_blank')}
                            className="p-3 bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 hover:opacity-90 rounded-full transition-all duration-300 transform hover:scale-110"
                        ...
                        {/* Twitter/X Share */}
                        <button
                            onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent('¡Bira Party te invita a asistir a su evento!')}&url=${encodeURIComponent('https://biraparty.lat')}`, '_blank')}
                            className="p-3 bg-black hover:bg-gray-800 border border-white/20 rounded-full transition-all duration-300 transform hover:scale-110"
