...
import { adminService, CodigoCanjeado } from '@/services/admin';
import { format } from 'date-fns';
import { MobileNav } from '@/components/MobileNav';
import { es } from 'date-fns/locale';

export default function CanjeadosPage() {
    const router = useRouter();
    const [codigos, setCodigos] = useState<CodigoCanjeado[]>([]);
    ...
    const loadCodigos = async () => {
        try {
            const data = await adminService.getCodigosCanjeados();
            setCodigos(data);
        } catch (error) {
        ...
        return (
            <div className="relative flex items-center justify-center min-h-screen w-full bg-black overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center gap-4">
                    ...
                    <p className="text-white text-sm">Cargando códigos canjeados...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen w-full bg-black text-white">
            ...
            <main className="w-full px-4 py-6">
                <h2 className="text-xl sm:text-2xl font-bold mb-6">Códigos Canjeados</h2>
                {/* Search Bar */}
                <div className="mb-6">
                    <input
                        type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
                        <p className="text-gray-400 text-sm">Total Canjeados</p>
                        <p className="text-3xl font-bold text-green-400">{codigos.length}</p>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
                        <p className="text-gray-400 text-sm">Validados en Evento</p>
                        <p className="text-3xl font-bold text-blue-400">
                            {codigos.filter(c => c.validadoEvento).length}
                        </p>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
                        <p className="text-gray-400 text-sm">Pendientes de Validar</p>
                ...
                <div className="hidden md:block bg-white/5 border border-white/10 rounded-xl overflow-hidden backdrop-blur-sm">
                        ...
                        <table className="w-full">
                            <thead className="bg-white/10">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Fecha Canje</th>
                            ...
                            <tbody className="divide-y divide-white/10">
                                {filteredCodigos.map((codigo) => (
                                    <tr key={codigo.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">
                                            {format(new Date(codigo.fechaCanje), 'dd/MM/yyyy HH:mm', { locale: es })}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            {codigo.validadoEvento ? (
                    ...
                    {filteredCodigos.length === 0 && (
                        <div className="text-center py-12 text-gray-400">
                            No se encontraron códigos canjeados
                        </div>
                    )}
                </div>
                ...
                <div className="md:hidden space-y-4">
                    {filteredCodigos.map((codigo) => (
                        <div key={codigo.id} className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
                            <div className="flex justify-between items-start mb-3">
                                <span className="px-3 py-1.5 bg-green-500/20 text-green-400 rounded-lg font-mono font-bold text-sm">
                                    {codigo.codigo}
                                </span>
                                {codigo.validadoEvento ? (
                                    <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-full text-xs font-semibold">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="font-semibold">{codigo.invitado?.nombres} {codigo.invitado?.apellidos}</p>
                                ...
                                <div>
                                    <p className="text-xs text-gray-400 mb-1">Fecha de Canje</p>
                                    <p className="text-sm">{format(new Date(codigo.fechaCanje), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                                </div>
                            </div>
                        </div>
                    ))}
